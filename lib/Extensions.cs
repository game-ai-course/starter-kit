...
{
    public static IEnumerable<IReadOnlyList<T>> AllSubsets<T>(this IEnumerable<T> items)
    {
        var list = items.ToList();
        var n = list.Count;
        var subsetsCount = 1 << n;
        for (var mask = 0; mask < subsetsCount; mask++)
        {
    ...
    public static V Average(this IEnumerable<V> items)
    {
        var sum = V.Zero;
        var count = 0;
        foreach (var item in items)
    ...
    public static V Average<T>(this IEnumerable<T> items, Func<T, V> getV)
    {
        var sum = V.Zero;
        var count = 0;
        foreach (var item in items)
    ...
    public static int BitsCount(this int x) => BitOperations.PopCount((uint) x);

    public static int GetBit(this int x, int bitIndex) => (x >> bitIndex) & 1;
    public static bool HasBit(this int x, int bitIndex) => ((x >> bitIndex) & 1) == 1;
    ...
    public static IList<T> AllMinBy<T>(this IEnumerable<T> items, Func<T, double> getKey)
    {
        IList<T> result = null;
    ...
    public static string StrJoin<T>(this IEnumerable<T> items, string delimiter, Func<T, object> toString)
    {
